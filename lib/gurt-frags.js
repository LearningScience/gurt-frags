...
exp['Linter'] = (stream, filter) => stream
	.pipe(filter(['**/*.js', '!libs/**/*']))
	.pipe(plugin.eslint())
...
exp['Post-Process'] = (stream, filter) => stream
	...
	.pipe(plugin.csso())
	.pipe(filter('**/*.js'))
	.pipe(plugin.babel({presets: [[mod + '/node_modules/babel-preset-env', {modules: false}]]}))
...
exp['build'] = 'Linter.Pre-Process.Bundle.Concat.Revision.Inject.Post-Process';
exp['serve'] = 'Linter.Pre-Process.Inject';
